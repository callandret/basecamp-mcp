...
export async function getAccessToken(
  ...
  if (fs.existsSync(TOKEN_FILE)) {
    const tokens: TokenData = JSON.parse(fs.readFileSync(TOKEN_FILE, 'utf-8'));
  ...
  return new Promise((resolve, reject) => {
    ...
    app.get('/callback', async (req, res) => {
      ...
      try {
        ...
        const bc4Account = authData.accounts.find((a: any) => a.product === 'bc3');
...
async function refreshAccessToken(
  ...
  const existingTokens: TokenData = JSON.parse(fs.readFileSync(TOKEN_FILE, 'utf-8'));
